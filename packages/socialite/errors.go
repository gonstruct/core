...
var (
	...
	// half-built.
	ErrNotConfigured = errors.New("socialite: driver is not configured")
	...
	// caller on purpose.
	ErrNoHandshake = errors.New("socialite: no handshake for this callback")

	// ErrStateMismatch means the state did not match the one issued, which is
	// what RFC 6749 section 10.12 asks a client to check.
	ErrStateMismatch = errors.New("socialite: state does not match")
	...
	ErrIssuerMismatch = errors.New("socialite: the response came from another issuer")

	// ErrExchange means the authorization code could not be exchanged, or what
	// came back was not a token this package can use.
	ErrExchange = errors.New("socialite: failed to exchange the authorization code")

	// ErrProfile means the provider would not describe the user.
	ErrProfile = errors.New("socialite: failed to read the profile")

	// ErrUnknownDriver means no provider is registered under that name.
	ErrUnknownDriver = errors.New("socialite: unknown driver")
	...
	ErrAuthorization = errors.New("socialite: the provider refused the authorization request")
...
//
// Socialite lets this case fall through to a failed exchange, which reports a
// cancelled sign-in as a broken one. It is an answer, so it is modelled.
