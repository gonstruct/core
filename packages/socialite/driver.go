...
// belong to this request rather than to the provider -- a login_hint, a
// prompt -- and implement Parameterised for the ones that are always sent.
//
...
// and nonce on the way out. It returns a response so a controller can return it
// directly, including when the provider has no credentials.
func (self *Flow) Redirect(redirectTo string) response.Response {
	if self.provider == nil {
		return response.Error(
			fmt.Errorf("%w: %s", ErrUnknownDriver, self.name),
			response.WithMessage("That sign-in provider does not exist"),
		)
	}

	if !self.provider.Configured() {
		return response.Unprocessable(
			response.WithMessageF("%s sign-in is not configured", self.name),
			response.WithCode(self.name+"_oauth_unconfigured"),
		)
	}

	url, err := self.AuthorizationURL(redirectTo)
	if err != nil {
		return response.Error(err, response.WithMessage("Failed to start sign-in"))
	}

	return response.Redirect(http.StatusFound, url)
}

// AuthorizationURL is Redirect without the redirecting, for a caller that hands
// the URL to a client rather than to a browser. It still writes the handshake,
// so whatever follows the URL must be the browser this was called for.
func (self *Flow) AuthorizationURL(redirectTo string) (string, error) {
	...
	if !self.stateless {
		issued, err := self.cookie.begin(self.context, self.sealer, redirectTo, self.usesPKCE(), self.usesNonce())
		if err != nil {
			return "", err
		}
...
func (self *Flow) User() (*User, error) {
	...
	return self.profile(Grant{Token: token, Nonce: handshake.Nonce})
}
...
func (self *Flow) UserFromToken(token *oauth2.Token) (*User, error) {
	...
	return self.profile(Grant{Token: token})
}
...
func (self *Flow) verified() (Handshake, error) {
	...
	handshake, ok := self.cookie.end(self.context, self.sealer)
	if !ok {
	...
	returned := self.context.Query("state")
	if subtle.ConstantTimeCompare([]byte(handshake.State), []byte(returned)) != 1 {
...
func (self *Flow) issuer() error {
	...
	if self.context.Query("iss") != expected.Issuer() {
...
func (self *Flow) refusal() error {
	...
	return &AuthorizationError{
		Code:        code,
		Description: self.context.Query("error_description"),
		URI:         self.context.Query("error_uri"),
	}
}
...
func (self *Flow) exchange(handshake Handshake) (*oauth2.Token, error) {
	...
	token, err := self.config().Exchange(self.context, self.context.Query("code"), options...)
	if err != nil {
		return nil, fmt.Errorf("%w: %w", ErrExchange, err)
	}
...
func (self *Flow) profile(grant Grant) (*User, error) {
	user, err := self.provider.User(self.context, grant)
	if err != nil {
		return nil, fmt.Errorf("%w: %w", ErrProfile, err)
	}

	user.Token = grant.Token
	user.ApprovedScopes = granted(grant.Token, self.Scoped())
