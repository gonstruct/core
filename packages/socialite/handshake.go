...
// redirect and the callback. The application supplies it: choosing an
// encryption provider is not this package's business.
...
// mirror the application's own session cookie so the two behave alike.
...
func (self CookieOptions) write(context *gin.Context, value string, maxAge int) {
	context.SetSameSite(self.SameSite)
	context.SetCookie(self.Name, value, maxAge, self.Path, self.Domain, self.Secure, true)
}
...
func (self CookieOptions) begin(
	...
	self.write(context, sealed, int(self.Lifetime.Seconds()))
...
func (self CookieOptions) end(context *gin.Context, sealer Sealer) (Handshake, bool) {
	defer self.write(context, "", -1)

	cookie, err := context.Cookie(self.Name)
	if err != nil || cookie == "" {
		return Handshake{}, false
	}

	payload, err := sealer.Open(cookie)
	if err != nil {
		return Handshake{}, false
	}
...
func randomToken(size int) string {
	...
	if _, err := rand.Read(buffer); err != nil {
		...
		panic("socialite: failed to read random bytes: " + err.Error())
	}
